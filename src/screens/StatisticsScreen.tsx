...
export function StatisticsScreen() {
  ...
  const loadStats = async () => {
    ...
    // Region stats
    const regions = new Map<string, { total: number; levels: number[]; mastered: number }>();
    for (const muscle of MUSCLES) {
      const entry = regions.get(muscle.bodyRegion) ?? { total: 0, levels: [], mastered: 0 };
      entry.total++;
  ...
  const getMasteryColor = (level: number): string => {
    if (level >= 3.5) return Colors.mastery[4];
    if (level >= 2.5) return Colors.mastery[3];
    if (level >= 1.5) return Colors.mastery[2];
    if (level >= 0.5) return Colors.mastery[1];
    return Colors.mastery[0];
  };
        ...
        <View style={styles.overallCard}>
          <View style={styles.overallRow}>
            ...
            <View style={styles.overallItem}>
              ...
              <Text style={styles.overallLabel}>숙달</Text>
            </View>
            <View style={styles.overallItem}>
              ...
              <View style={styles.regionBar}>
                <View
                  style={[
                    ...
                    {
                      width: `${(stat.avgLevel / 4) * 100}%`,
                      backgroundColor: getMasteryColor(stat.avgLevel),
                    },
                  ]}
                />
              </View>
